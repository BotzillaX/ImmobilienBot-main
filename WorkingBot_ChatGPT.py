...
def wechselDesBrowsers():
    ...
    ## 12
def zusammensetzungDerNachricht(Name, Adresse, Info, mieter):
    prompt = f"act as an apartment seeker, you are interested in all apartments, your name is {mieter["name"]}, you are {mieter["alter"]} years old, you live in {mieter["geburtsort"]}, you write your cover letters only in german, your cover letter always starts with \"Sehr geehrte/geehrter/geehrtes\", your cover letters are random, write depending on the following information: {Name}, Adresse: {Adresse}, additional Information: {Info}"
  
    
    max_loc, max_val = lookingForPixel("GPT")
    ...
    time.sleep(Legende["averageTimeGeneral"]+1)
                ...
                if statusHearth == "noHearth":
                    ...
                    if hearth == False:
                        ...
                        informationenÜberWohnung = {
                            ...
                            "information der Wohnung": " "+ Wohnungsinfo()
                             ## 10 
                            }
                        wechselDesBrowsers()
                        time.sleep(Legende["averageTimeGeneral"])
                        zusammensetzungDerNachricht(informationenÜberWohnung["Name des Vermieters"], informationenÜberWohnung["Standort der Wohnung"],informationenÜberWohnung["information der Wohnung"], eigeneDaten)
                        time.sleep(Legende["averageTimeGeneral"])
                        beenden()
                        ExtraktionAusGPT()
                        #ExtraktionAusGPT() ## 13
